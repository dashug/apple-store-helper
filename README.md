...
## 功能说明
### 界面
左栏放地区、门店与型号，右侧整片留给监听列表，顶部工具栏放设置、有货记录、
打开日志与开始/暂停，底部一条状态栏。窗口拉大时长的是列表 —— 盯十几家店时
最需要的就是列表高度。左栏中间与两栏之间的分隔条都可以拖动。

通知地址、监听间隔、命中后行为都在工具栏的`设置`里。它此前是左栏的折叠面板，
展开时会把界面最小高度顶到 937，1366×768 的笔记本放不下，点开的瞬间窗口
还会被强行撑大。

**不可逆的操作都要确认**：`清空当前地区`会先说明要清掉多少项，行内的删除是
一个低调的图标按钮，与`停用`之间隔开，点下去也会确认。此前这两个按钮
分别与`添加`、`停用`同宽并排，误触的代价和它们完全不对等。

### 查看与操作
* 监听列表上方可按状态`筛选`（全部 / 有货 / 未知 / 无货 / 等待），只影响显示，
  不影响监听范围；筛掉部分时会标出「N / M 项」
* 状态栏显示`状态 · 项数 · 上轮完成时间 · 下一轮倒计时`，可据此确认程序仍在
  正常轮转；退避时两轮之间可能隔几分钟，倒计时能区分「在等」和「卡死」
* 窗口尺寸与两条分隔条的位置都会被记住，下次启动恢复 —— 门店多还是型号多
  因人而异，拖好的分配不该每次重来
...
### 命中后的行为
同一轮里所有有货的项都会被报告，而不是只报第一个 —— 两家店同时有货时，
用户需要据此决定去哪一家。通知与有货记录也会包含全部命中。

默认命中即暂停监听。勾选`命中后继续监听其余项`可改为继续：盯十几项时，
命中的那家未必是你去得了的，其余项不该跟着停。
...
**每轮的请求数等于门店数**：盯三家店、5 秒一轮，就是每秒 0.6 个请求。
实测这个强度跑约 70 秒（40 次请求）后就会收到 HTTP 541，全部转为`未知`，
随后自动退避。门店多时请把间隔调大。

退避分两层，因为「一家店坏了」和「接口挂了」是两回事：

* **整轮所有门店都失败**才算全局问题（限流或接口下线），这时才拉长轮询间隔，
  上限 5 分钟，恢复正常后立即回到设定的间隔
* **个别门店连续失败 3 次**则只暂停查询那一家，间隔从 30 秒起翻倍，上限 5 分钟；
  该门店一次查询成功即恢复，不需要手动干预

这样列表里留一个已停用的门店编号，不会把其余健康门店的检查频率一起拖垮。
被暂停查询的门店在列表里显示`未知`，原因栏会写明暂停到几点。
