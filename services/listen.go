...
func (s *listenService) getSkuByLink(ch chan map[string]bool, skUrl string) {
    ...
    resp, body, errs := gorequest.New().
        Get(skUrl).
        Set("referer", fmt.Sprintf("https://www.apple.com/%s/shop/buy-iphone", s.Area.ShortCode)).
...
func (s *listenService) SendPushNotificationByBark(title string, content string, bagUrl string) {
    ...
    base := strings.TrimRight(s.BarkNotifyUrl, "/")
    apiUrl := fmt.Sprintf("%s/%s/%s?url=%s",
        base,
        url.PathEscape(title),
        url.PathEscape(content),
        url.QueryEscape(bagUrl),
    )

    response, err := http.Get(apiUrl)
    if err != nil {
        log.Println("Bark notify failed:", err)
        return
    }
